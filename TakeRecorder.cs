...
namespace Unity.LiveCapture
{
    ...
    public class TakeRecorder : MonoBehaviour, ITakeRecorderInternal
    {
        ...
        public void StartRecording()
        {
            ...
            string result = sceneName + "_" + timestamp;

            //rename actor here as well 
            //  ActorRenamer
            if(ActorObject != null)
            {
                ActorObject.name = result;
            }
            else
            {
                Projector projector = FindObjectOfType<Projector>();
                ActorObject = projector.gameObject;
                ActorObject.name = result;
            ...
            slate.ShotName = result;

            Debug.Log("Audiosource Play " + " " + result);
        ...
        Take ProduceTake()
        {
            ...
            if (AudioSource != null)
            {
                if (AudioSource.clip != null)
                {
                    slate.SceneNumber = AudioSource.clip.name;
                    slate.ShotName = m_Devices[0].name;

                    slate.Directory = "Assets/Takes/" + AudioSource.clip.name;

                    int minit = (int)DateTime.Now.Minute;
                    int secunds = (int)DateTime.Now.Second;

               

                    slate.TakeNumber = minit + secunds;
                }
            }
                ...
                m_Director))
            {
              ...
              //  Debug.Log("Produce take name" + slate.ShotName + " and take number " + slate.TakeNumber);
